  ...
  try {
    ...
    whatsappSocket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        const reasonCode = isBoom ? (lastDisconnect.error as Boom).output?.statusCode : 0;
        const shouldReconnect = reasonCode !== DisconnectReason.loggedOut;

        await sessionRef.update({ connected: false, updatedAt: new Date() });
        console.error(`[WARN] Connection closed (session: ${sessionName}). Reconnecting: ${shouldReconnect}`);

        if (shouldReconnect) {
          setTimeout(() => initializeWhatsApp(), 5000);
        } else {
          console.log(`[INFO] Session closed permanently. Delete /auth/${sessionName} to restart.`);
          whatsappSocket = null;
        }
      } else if (connection === 'open') {
        await sessionRef.set({ connected: true, qr: null, updatedAt: new Date() });
    ...
    console.error(`[ERROR] Failed to initialize WhatsApp "${sessionName}":`, error);
    throw error;
  }
}
...
export function getSocket() {
  if (!whatsappSocket) {
    throw new Error('WhatsApp socket not initialized. Run initializeWhatsApp first.');
  }
  return whatsappSocket;
}
