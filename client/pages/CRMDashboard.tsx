  ...
  return (
    <div className="flex h-full w-full flex-col bg-background">
      ...
      <div className="border-b border-border px-6 py-4">
        ...
        <p className="text-sm text-muted-foreground">Manage your customer relationships and interactions</p>
      </div>
      ...
      <div className="border-b border-border px-6 py-4 space-y-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
          <div className="flex-1">
            <label className="block text-sm font-medium text-foreground mb-2">Search Contacts</label>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="rounded-lg bg-secondary p-3">
            ...
            <p className="text-xl font-bold text-primary">{contacts.filter((c) => c.status === 'active').length}</p>
          </div>
          <div className="rounded-lg bg-secondary p-3">
            <p className="text-xs text-muted-foreground mb-1">Leads</p>
            <p className="text-xl font-bold text-yellow-500">{contacts.filter((c) => c.status === 'lead').length}</p>
          </div>
          <div className="rounded-lg bg-secondary p-3">
            <p className="text-xs text-muted-foreground mb-1">Favorites</p>
            <p className="text-xl font-bold text-foreground">{contacts.filter((c) => c.isFavorite).length}</p>
          </div>
        </div>
      </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-semibold text-foreground group-hover:text-primary">{contact.name}</h3>
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                      ...
                      >
                        <Star
                          className={`h-4 w-4 ${
                            contact.isFavorite ? 'fill-yellow-500 text-yellow-500' : 'text-muted-foreground'
                          }`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                      ...
                      {contact.company && (
                        <span className="rounded bg-secondary px-2 py-1">{contact.company}</span>
                      )}
                    </div>
